...
def is_allowed(user_id: int) -> bool:
    # Если список пустой, бот временно доступен всем.
...
def remove_last_tip(user_id: int) -> int | None:
    # Одна транзакция предотвращает случайное удаление неправильной записи.
...
async def deny_access(update: Update) -> None:
    ...
    if update.effective_message:
        await update.effective_message.reply_text(
            ...
            "Отправьте этот ID владельцу бота."
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "Каждое нажатие сразу добавляется в общий подсчёт.\n\n"
        f"Ваш Telegram ID: {user.id}",
