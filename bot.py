...
def get_all_participants() -> list[dict]:
    ...
    with get_connection() as connection:
        ...
        adjustment_rows = connection.execute(
            """
            SELECT target_user_id AS user_id, MAX(target_full_name) AS full_name
            FROM tip_adjustments
            WHERE target_user_id = ANY(%s)
        ...
        rows = connection.execute(
            ...
            FROM (
                SELECT
                    created_at_utc,
                    amount_cents,
                    full_name AS actor_full_name,
                    'Добавил себе' AS action
                FROM tips
                WHERE user_id = %s
...
def clear_history_context(context: ContextTypes.DEFAULT_TYPE) -> None:
    context.user_data.pop("history_targets", None)
...
async def begin_history(
    ...
    for participant in participants:
        ...
        # Если имена совпадают, ID поможет отличить участников.
        if label in target_map:
            previous = target_map.pop(label)
            previous_label = f"{previous['full_name']} [{previous['user_id']}]"
            target_map[previous_label] = previous

            for row in rows:
                if row == [label]:
                    row[0] = previous_label
                    break
    ...
    rows.append(["Отмена"])
    context.user_data["history_targets"] = target_map

    await update.message.reply_text(
        "Выберите участника, историю которого хотите посмотреть:",
        reply_markup=ReplyKeyboardMarkup(rows, resize_keyboard=True),
    )
    return HISTORY_SELECT_PERSON


async def select_history_person(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> int:
    text_value = update.message.text

    if text_value == "Отмена":
    ...
    target = context.user_data.get("history_targets", {}).get(text_value)
    if not target:
        await update.message.reply_text("Выберите участника кнопкой ниже.")
        return HISTORY_SELECT_PERSON

    target_user_id = int(target["user_id"])
    target_full_name = target["full_name"]
    history = get_user_history(target_user_id, limit=20)
    today, month, all_time = get_user_totals(target_user_id)

    lines = [
        f"📜 История: {target_full_name}",
        "",
        f"Сегодня: {money(today)}",
    ...
    if not history:
        ...
        for row in history:
            ...
            local_time = created_at.astimezone(LOCAL_TZ)
            amount_cents = int(row["amount_cents"])
            sign = "+" if amount_cents > 0 else "−"
            amount_text = money(abs(amount_cents))
            actor_name = row["actor_full_name"]
            action = row["action"]

            lines.append(
                f"• {local_time.strftime('%m/%d %I:%M %p')} — "
                f"{sign}{amount_text}\n"
                f"  {action}: {actor_name}"
            )
...
def build_application() -> Application:
    ...
    history_handler = ConversationHandler(
        entry_points=[
            MessageHandler(
                filters.Regex(r"^История$"),
                begin_history,
            )
        ],
        states={
            HISTORY_SELECT_PERSON: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, select_history_person)
            ],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
        allow_reentry=True,
